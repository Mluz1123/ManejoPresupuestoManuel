...
namespace ManejoPresupuesto.Controllers
{
    ...
    public class TransaccionesController: Controller
    {
        ...
        public TransaccionesController(IServicioUsuarios servicioUsuarios, IRepositorioCuentas repositorioCuentas, 
            IRepositorioCategorias repositorioCategorias, IRepositorioTransacciones repositorioTransacciones, IMapper mapper, IServicioReportes servicioReportes)
        {
        ...
        {
            var usuarioId = servicioUsuarios.ObtenerUsuarioId();
            IEnumerable<ResultadoOptenerPorSemana> transaccionesPorSemana = await servicioReportes.ObtenerReporteSemanal(usuarioId, mes, año, ViewBag);

            var agrupado = transaccionesPorSemana.GroupBy(x => x.Semana).Select(x => new ResultadoOptenerPorSemana()
            {
                Semana = x.Key,
            ...
            for(int i = 0; i < diasSegmentados.Count(); i++)
            {
                ...
                {
                    agrupado.Add(new ResultadoOptenerPorSemana()
                    {
                        Semana = semana,
        ...
        //Devuelve al usuario donde se encontraba string urlRetorno = null
